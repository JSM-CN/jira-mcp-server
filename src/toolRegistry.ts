      ...
      switch (toolName) {
        ...
        case 'get_boards':
          return await this.boardService.getBoards({
            type: validateStringInput(args.type, 'type', { maxLength: 20 }),
            projectKey: validateProjectKey(args.projectKey),
          });
        case 'get_board_details':
          return await this.boardService.getBoardDetails(
            validateStringInput(args.boardId, 'boardId', { required: true, maxLength: 50 })
          );
        case 'get_board_issues': {
          const maxResults = validateNumberInput(args.maxResults, 'maxResults', { min: 1, max: 100 });
          return await this.boardService.getBoardIssues({
            boardId: validateStringInput(args.boardId, 'boardId', { required: true, maxLength: 50 }),
        ...
        case 'search_issues': {
          const maxResults = validateNumberInput(args.maxResults, 'maxResults', { min: 1, max: 100 });
          const startAt = validateNumberInput(args.startAt, 'startAt', { min: 0 });
          return await this.issueService.searchIssues({
            jql: sanitizeJQL(args.jql),
            ...(maxResults !== undefined && { maxResults }),
        ...
        case 'get_issue_details':
          return await this.issueService.getIssueDetails({
            issueKey: validateIssueKey(args.issueKey),
        ...
        case 'create_issue':
          return await this.issueService.createIssue({
            projectKey: validateProjectKey(args.projectKey),
            issueType: validateStringInput(args.issueType, 'issueType', { required: true, maxLength: 50 }),
            summary: validateStringInput(args.summary, 'summary', { required: true, maxLength: 255 }),
            description: validateStringInput(args.description, 'description', { maxLength: 50000 }),
            priority: validateStringInput(args.priority, 'priority', { maxLength: 20 }),
            assignee: validateStringInput(args.assignee, 'assignee', { maxLength: 100 }),
            labels: args.labels as string[],
            components: args.components as string[],
            fixVersions: args.fixVersions as string[],
            dueDate: validateStringInput(args.dueDate, 'dueDate', { maxLength: 20 }),
            parentKey: validateIssueKey(args.parentKey),
          });
        case 'update_issue':
          return await this.issueService.updateIssue({
            issueKey: validateIssueKey(args.issueKey),
            summary: validateStringInput(args.summary, 'summary', { maxLength: 255 }),
            description: validateStringInput(args.description, 'description', { maxLength: 50000 }),
            priority: validateStringInput(args.priority, 'priority', { maxLength: 20 }),
            assignee: validateStringInput(args.assignee, 'assignee', { maxLength: 100 }),
            labels: args.labels as string[],
        ...
        case 'transition_issue':
          return await this.issueService.transitionIssue({
            issueKey: validateIssueKey(args.issueKey),
            transitionName: validateStringInput(args.transitionName, 'transitionName', { required: true, maxLength: 100 }),
            comment: validateStringInput(args.comment, 'comment', { maxLength: 10000 }),
          });
        case 'add_comment':
          return await this.issueService.addComment({
            issueKey: validateIssueKey(args.issueKey),
            comment: validateStringInput(args.comment, 'comment', { required: true, maxLength: 10000 }),
          });
        ...
        case 'search_users':
          return await this.userService.searchUsers(
            validateStringInput(args.query, 'query', { required: true, maxLength: 200 })
          );
        case 'get_user_details':
          return await this.userService.getUserDetails(
            validateStringInput(args.accountId, 'accountId', { required: true, maxLength: 100 })
          );
        ...
        case 'get_project_details':
          return await this.projectService.getProjectDetails(
            validateProjectKey(args.projectKey)
          );
        ...
        case 'add_worklog':
          return await this.worklogService.addWorklog({
            issueKey: validateIssueKey(args.issueKey),
            timeSpent: validateStringInput(args.timeSpent, 'timeSpent', { required: true, maxLength: 20 }),
            comment: validateStringInput(args.comment, 'comment', { maxLength: 10000 }),
            startDate: validateStringInput(args.startDate, 'startDate', { maxLength: 30 }),
          });
        case 'get_worklogs':
          return await this.worklogService.getWorklogs(
            validateIssueKey(args.issueKey)
          );
